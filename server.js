...
// Netlify Functions run from a read-only deployment filesystem. Never use data.json there.
...
const seed = {
  settings: {
    storeName: 'Nepkits Hub',
    tagline: 'Jerseys. Streetwear. Nepal.',
    logo: '/nepkits-hub-logo.svg',
    paymentQr: '',
    phone: '',
    email: '',
    instagram: ''
...
function localRead() {
  if (isServerless) throw new Error('Local file storage is disabled in Netlify Functions.');
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    res.json(d.products.filter((x) => x.active !== false));
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!name || !normalizedEmail || !password || password.length < 6) return res.status(400).json({ message: 'Invalid registration' });
    if (d.users.some((x) => x.email === normalizedEmail)) return res.status(409).json({ message: 'Email already registered' });
    const u = { id: Date.now(), name: String(name).trim(), email: normalizedEmail, passwordHash: await bcrypt.hash(password, 10), role: 'customer' };
    d.users.push(u);
    await writeData(d);
...
app.get('/api/orders/my', auth, async (req, res) => {
  try { res.json((await readData()).orders.filter((o) => o.userId === req.user.id)); }
  catch (e) { res.status(500).json({ message: e.message }); }
});
...
app.post('/api/orders', auth, async (req, res) => {
  try {
    const d = await readData();
    const o = { id: 'NH-' + Date.now().toString().slice(-8), userId: req.user.id, date: new Date().toISOString(), customer: req.body?.customer, items: req.body?.items || [], total: Number(req.body?.total || 0), status: 'Pending review', paymentProof: req.body?.paymentProof || '' };
    d.orders.unshift(o);
    await writeData(d);
...
app.get('/api/admin/stats', auth, admin, async (req, res) => {
  try {
    const d = await readData();
    res.json({
      products: d.products.filter((x) => x.active !== false).length,
      orders: d.orders.length,
      customers: d.users.filter((x) => x.role === 'customer').length,
      revenue: d.orders.filter((x) => x.status !== 'Cancelled').reduce((s, x) => s + x.total, 0),
      pending: d.orders.filter((x) => !['Delivered', 'Cancelled'].includes(x.status)).length,
      lowStock: d.products.filter((x) => x.stock <= 5).length
    });
  } catch (e) { res.status(500).json({ message: e.message }); }
});
...
app.post('/api/admin/products', auth, admin, async (req, res) => {
  try {
    const d = await readData();
    const p = { id: Date.now(), name: req.body?.name, cat: req.body?.cat || 'Jerseys', price: Number(req.body?.price || 0), stock: Number(req.body?.stock || 0), image: req.body?.image || '', active: true };
    d.products.push(p);
    await writeData(d);
